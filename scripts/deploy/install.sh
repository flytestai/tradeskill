...
# ---- MCP 端点鉴权（2026-09-19 新增）----
# 背景：实测该端点此前**完全不校验凭据**（无 Key/伪造 Key 均 200），
#       而同容器 REST 无 Key 返回 401。
#
# ⚠️ 默认 warn 而不是 enforce —— 这是踩过坑后的选择：
#    现有 MCP 客户端（蜜蜂网关等）在此端点上**不发任何凭据**。
#    本会话曾直接上 enforce，部署后公网入口立刻整体 401，比"无鉴权"更糟。
#    故默认只**记录**不拦截（风险可见但不破坏服务）。
#
# warn（默认）= 记录谁在无凭据访问，不拦截
# enforce     = 拒绝公网来源；回环/私网放行
#   ⚠️ 切换前必须确认：所有 MCP 客户端都能带 Key，
#      且 nginx 已配置凭据透传（见 nginx-skill.conf 的 /mcp 段）
# strict      = 任何来源都必须带 Key（要求同上，更严）
PLATFORM_MCP_AUTH_MODE=warn
