...
- **VIP 消息实时推送**：内容含 `【仅TA的真爱粉可见】` 的消息判定为 VIP 消息，入库后立即以 Card 2.0 卡片推送到「**荔枝种植交流群**」（紫色卡片）和「**每日复盘群**」（青绿卡片），卡片标题统一为「👑 VIP 尊享・仅 TA 的真爱粉可见」，时间只在卡片副标题显示一次；两个群均只发 VIP，公开微博不再转发。推送失败会记录并自动补推，同时发私信告警。VIP 标记词可在 `local_config.env` 用 `VIP_MARKERS`（逗号分隔）覆盖
...
**其它提醒**：
- **盘前播报**：每个交易日 08:45 自动以飞书 Card 2.0 卡片私信发送，包含纳斯达克100最新价、涨跌幅、当前PE、距历史高点回撤，以及综合量化评分、建议仓位和加减仓条件；行情按东方财富→腾讯双源降级。
- **量化综合评估**：盘前同时输出「超短线（1～3日）」和「波段（1～4周）」两套评分；超短线弱化PE/PB，重点看动量、ETF走势、关键位和溢价变化，波段再提高回撤和中期趋势权重。默认 1 层=20%，ETF溢价率、MA5、中短期走势、ATR14和5/20日波动率作为实际交易约束；波动放大时自动限制最大仓位。纳指估值优先使用公开QQQ代理数据；公开源缺失的PB/百分位显示缺失，不再回退到好人好股估值接口。
- **纳指ETF评估**：同步读取纳指ETF易方达（159696）的A股最新价、涨跌幅、近5日/近20日走势、MA5和溢价率，单独判断是否适合加仓；指数估值偏低但ETF溢价过高时，明确提示“不适合直接加仓，等待溢价回落”。
- **美股时间口径**：纳斯达克100显示“美股最近交易日”，不直接展示可能混淆时区的原始行情时间；盘前播报在A股交易日读取最近一笔美股可用行情。
- **盘前播报格式**：采用 Markdown 分区展示「纳指行情 → 当前PE → 高点回撤 → ETF走势/溢价 → 超短线建议 → 波段建议 → ETF综合建议/仓位上限 → 纳指支撑/阻力 → 免责声明」；回撤按当前价相对历史最高价计算。
- **午间汇总**：每个交易日 11:35 自动生成上午汇总，私信发送给用户
- **收盘汇总**：每个交易日 15:05 自动生成当日汇总，私信发送给用户
...
`scripts/price_alerts.py` 支持「标的到某价 / 跌破 / 突破 / 区间」自动提醒：盘中后台循环每 30 秒查一次价，触发后通过飞书私信发送给用户。
